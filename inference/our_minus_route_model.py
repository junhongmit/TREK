...
PROMPTS["topic_entity"] = {
    ...
    -Goal-
    You are presented with a question in the {domain} domain, and its query time.
    
    1) Determine the topic entities asked in the query. The topic entities will be used as source entities to search through a knowledge graph for answers.
...
PROMPTS["align_topic"] = {
    ...
    The entities' relevance would be scored on a scale from 0 to 1 (the sum of the scores of all entities is 1). 
    ...
    *NEVER include ANY EXPLANATION or NOTE in the output, ONLY OUTPUT JSON*  
    ...
    Output: {{"reason": "The question asks about the movie 'Inception', and ent_0 is the entity that directly corresponds to the movie 'Inception'.", "relevant_entities": {{"ent_0": 1}}}}
    ...
    Output: {{"reason": "The entity ent_0 is the award for the best animated feature film in the year of query time, 2024, asked in the question.", "relevant_entities": {{"ent_0": 1}}}}
    ...
    Output:{{"reason": "The question requests the 2018 best picture Oscar movies, and award ent_2 is for the best picture in 2018. The award ent_0 is for the actress in a leading role in 2018, which may also help answer the question.", "relevant_entities": {{"ent_2": 0.8, "ent_0": 0.1, "ent_3": 0.1}}}}
    """),
...
PROMPTS["relations_pruning"] = {
    ...
    The goal is to retrieve up to {width} relations that contribute to answering the question. Rate their relevance from 0 to 1 (the sum of the scores of these relations is 1).
...
PROMPTS["triplets_pruning"] = {
    ...
    The goal is to score the relations' contribution to answering the question. Rate them on a scale from 0 to 1 (the sum of the scores of all relations is 1).
...
PROMPTS["validation"] = {
    ...
    A confident but careless friend has provided us a tentative answer, denote as "attempt". We don't really trust it, so we have retrieved a list of potential associated retrieved knowledge graph entities and triplets (entity, relation, entity), and provided tentative answer.
    ...
    You will act as a rigorous judge to whether the answers reach a consensus or not. Consensus is defined by at least a half of the answers (including my friend's attempt) agree on a specific answer.
    ...
    2. If there is not consensus, look into the answer carefully. You have to respond with "Yes", and summarize them into a final answer following with a summarized explanation.
    ...
    3. Lastly, if none of the attempts give a resonable answer (all "I don't know"), then fall back to use my friend's attempt.                           
...
class OurMinusRoute_Model:
    ...
    async def align_topic(
        ...
        for idx, topic in enumerate(topic_entities):
            ...
            results.extend([
                RelevantEntity(top_k_entities_dict[ind], norm_coeff * score)
                for ind, score in topic_entities_score.items()
                if (score > 0) and (ind in top_k_entities_dict)
            ])
        return results
    ...
    async def validation(
        ...
        user_message += f"Our Answer: {queries[0].subqueries}\n" + \
            "Reference: " + results[0]["context"] + '\n' + \
            "Answer: " + results[0]["ans"] + '\n\n'
        user_message += "Output:"
